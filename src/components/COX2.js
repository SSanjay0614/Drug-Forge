...
const COX2 = () => {
  ...
  const handlePredictClick = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5006/predict", {
        method: "POST",
        headers: {
  ...
  return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        <h1 style={titleStyle}>COX-2 Inhibitor Prediction</h1>
        <h2 style={subTitleStyle}>Predict the inhibitory activity of a compound on COX-2 enzyme</h2>
        <p style={infoStyle}>
          COX-2 (Cyclooxygenase-2) is an enzyme responsible for the production of prostaglandins, which are involved in inflammation and pain. Inhibitors of COX-2 are used as nonsteroidal anti-inflammatory drugs (NSAIDs) to reduce inflammation and pain.
        </p>
        <form onSubmit={handlePredictClick} style={formStyle}>
          ...
          >
            {isLoading ? "Predicting..." : "Predict COX-2 Inhibitor"}
          </button>
        </form>
          ...
          <div style={resultStyle}>
            ...
            <p style={resultTextStyle}>
              {prediction.predictedClass === 1
                ? "The compound is likely to be a COX-2 inhibitor."
                : "The compound is unlikely to be a COX-2 inhibitor."}
            </p>
          </div>
        )}
      </div>
